...
class TitanMasterEngine:
    ...
    def execute_consensus_arbitrator(self, list_a, list_b):
        ...
        2. If they disagree on a match, discard it. 
        3. If there are ZERO matches that agree, you must output exactly the word: ZERO_CONSENSUS
        4. If there is agreement, format the surviving matches beautifully for Telegram. Use clear emojis.
        5. CRITICAL: Provide ZERO explanations or intro text. Just the final agreed raw predictions.
        
        Format for surviving matches:
    ...
    async def run_pipeline(self):
        ...
        # Intercept the "None" or "ZERO_CONSENSUS" output to format a proper safety message
        if "ZERO_CONSENSUS" in final_ticket or final_ticket.lower() == "none" or not final_ticket:
            ...
            final_msg = final_ticket # Passes through the exact error message if an API fails
        else:
