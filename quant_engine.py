...
class ConsensusEngine:
    ...
    def fetch_and_scrape_sync(self, site_name, cfg):
        try:
            ...
            if site_name == "PredictZ":
                row_target = re.compile("pttr|ptrow")
                
            rows = soup.find_all(cfg["row_selector"], class_=row_target)

            if not rows:
                self.diagnostics[site_name] = f"🟡 BLOCKED OR NO DATA"
                return
                ...
                try:
                    ...
                    if site_name == "PredictZ":
                        ...
                        if len(links) >= 2:
                            home = links[0].text
                            away = links[1].text
                        picks = row.find_all("div", text=re.compile(r'^(1|X|2|Home|Away|Draw)', re.I))
                        if picks: pick = picks[0].text
                    else:
    ...
    def ask_llm_to_optimize_tickets(self, consensus_list):
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                self.diagnostics["AI_Handshake"] = f"🟢 Connected ({model_name.split('/')[-1]})"
            else:
    ...
    async def run_pipeline(self):
        ...
        # Increased to 4 workers since we now have 4 data sources
